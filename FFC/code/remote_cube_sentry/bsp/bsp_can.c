...
void DM_MotorDecode(DM_Motor *motor,uint8_t can_type,uint32_t can_id,uint8_t * can_msg)
{
    if(can_type == CAN_1)
    {
        if (0x13 == can_id) {
            ...
            Motor.t_int=((can_msg[4]&0xF)<<8)|can_msg[5];
            Motor.position = uint_to_float(Motor.p_int, P_MIN, P_MAX, 16); // (-12.5,12.5)
            Motor.velocity = uint_to_float(Motor.v_int, V_MIN, V_MAX, 12); // (-45.0,45.0)
            Motor.torque = uint_to_float(Motor.t_int, T_MIN, T_MAX, 12); // (-18.0,18.0)
